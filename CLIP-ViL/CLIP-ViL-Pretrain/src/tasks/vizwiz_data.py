...
Split2ImgFeatPath = {
    'train': 'data/vizwiz/vizwiz_imgfeat/train_obj36.h5',
    'valid': 'data/vizwiz/vizwiz_imgfeat/val_obj36.h5',
    "test": 'data/vizwiz/vizwiz_imgfeat/test_obj36.h5',
}
...
class VQADataset:
    ...
    def __init__(self, splits: str):
        ...
        for split in self.splits:
            self.data.extend(json.load(open("data/vizwiz/%s.json" % split)))
...
from src.pretrain.lxmert_vizwiz_data import ImageReader
"""
...
class VQATorchDataset(Dataset):
    def __init__(self, dataset: VQADataset):
        ...
        if not os.path.exists("data/vizwiz/width_heigths.json"):
            w_h_records = {}

            root_name = "/home/datasets/VizWiz/val/"
            ...
            root_name = "/home/datasets/VizWiz/train/"
            for root, dirs, files in os.walk(root_name, topdown=False):
                for file in tqdm(files):
                    image = np.asarray(Image.open(os.path.join(root_name, file)))
                    w = image.shape[0]
            ...
            root_name = "/home/datasets/VizWiz/test/"
            for root, dirs, files in os.walk(root_name, topdown=False):
                for file in tqdm(files):
                    image = np.asarray(Image.open(os.path.join(root_name, file)))
                    w = image.shape[0]
            ...
            with open("data/vizwiz/width_heigths.json", "w") as f:
                json.dump(w_h_records, f)
            assert(0)
        else:
            with open("data/vizwiz/width_heigths.json") as f:
        ...
        if self.input_raw_images:
            ...
            for split in dataset.splits:
                if "train" in split:
                    new_split.append("vizwiz_train")
                elif "val" in split:
                    new_split.append("vizwiz_val")
                else:
                    new_split.append("vizwiz_test")
            print(new_split)
    ...
    def get_height_and_width(self, index):
        ...
        img_id = datum['img_id']
        if "val" in img_id:
            image_file_name = "/home/datasets/VizWiz/val/{}.jpg".format(img_id)
        elif "train" in img_id:
            image_file_name = "/home/datasets/VizWiz/train/{}.jpg".format(img_id)
        elif "test" in img_id:
            image_file_name = "/home/datasets/VizWiz/test/{}.jpg".format(img_id)
