                ...
                for batch in dataloader:
                    ...
                    with CompleteInTimeOrDie(600 if loaded_batches else 3600 * 24):
                        ...
                        with torch.cuda.amp.autocast(enabled=self.training_config.fp16):
                            model_output = self.model(prepared_batch)
                        save_logits.append(model_output['scores'])
                        report = Report(prepared_batch, model_output)
                        reporter.add_to_report(report, self.model)
                        report.detach()
                save_logits = torch.cat(save_logits, axis=0).cpu().numpy()
                print(save_logits.shape)
                np.save("./ensemble/mcan_clip_kfold/mcan_clip_test_trainval_seed19960303_logits.npy", save_logits)
                reporter.postprocess_dataset_report()
