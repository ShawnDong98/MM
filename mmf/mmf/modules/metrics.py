...
class VQAAccuracy(BaseMetric):
    ...
    **Key**: ``vqa_accuracy``.
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        print(sample_list["targets"])
        print(sample_list["targets"].shape)
        print(sample_list["image_id"])
        output = model_output["scores"]
        # np.save("./ensemble/mcan_clip_kfold/mcan_clip_fold4_val_logits.npy", output.cpu().numpy())
        # for three branch movie+mcan model
        if output.dim() == 3:
            output = output[:, 0]
        expected = sample_list["targets"]
        # np.save("./ensemble/fold4_expected.npy", expected.cpu().numpy())
